...
\begin{abstract}
  ...
  Observing performance metrics from prior research showed that the performance of fine-tuned multilingual neural models is very close to the performance of monolingual ones.
...
Multilingual neural models simplify scalability and applications where many languages are needed for Natural Language Processing (NLP) tasks, for instance in an application setting with limited resources.
...
\begin{itemize}
\item BSNLP: 3rd Shared Task on SlavNER~\cite{piskorski-etal-2021-slav} submitted paper by ~\citet{prelevikj-zitnik-2021-multilingual}.
\item Clipping industry project developed by the Department of Knowledge Technologies at Jo\v{z}ef Stefan Institute~\cite{KTIJS}
\end{itemize}
...
The question that naturally arises from prior work, that this paper tries to answer, is this: did the fine-tuning with related languages influence the performance of a multilingual model compared to fine-tuning only in the target language?
...
For example, the BSNLP corpus uses PRO and EVT tags, while the wann corpus lacks a MISC tag common to 500k training corpora.
...
The third step was to remove sentences longer than 128-word tokens. 
The aim was to observe how many sentences would probably be clipped by the Transformer's tokenizers and not used by the fine-tuning algorithm.
The number of removed sentences (54) was insignificant compared to the corpora size.
%We could even dispute, that all the removed sentences are not proper sentences, for instance: newspaper crossword solutions presented as a single sentence or sentences missing ending punctuation.

In the fourth step, we converted corpora from standard CoNLL format to CSV format with two fields:
\begin{itemize}
...
\begin{table}[H]
  ...
  \end{tabular}
\end{table}
The fifth and final step was splitting the corpus data into train, test, and validation sets.
...
\begin{itemize}
  ...
  \item 40 epochs (preliminary runs showed that best F1-scores were produced between epoch 15 and 35)
  \item F1-score for best model selection and training progression.
\end{itemize}

\section{Evaluation}
\label{sec:evaluation}
In the following section, we define the F1-score we used for evaluation, and then we present the experiment results: the evaluation of the pre-trained multilingual model, followed by the evaluation of fine-tuning for each language.
...
We propose further investigating how performance changes, when distantly related languages are used for fine-tuning the models. 
This would further benefit the usage in an industrial setting if the performance is not degraded.
