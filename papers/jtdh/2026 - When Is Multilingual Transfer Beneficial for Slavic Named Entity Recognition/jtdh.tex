...
We adopted the learning rate and scheduler configuration of \textcite{piskorski-etal-2024-cross} to retain comparability with their Slavic NER benchmark, rather than as the result of an \texttt{mmBERT}-specific hyperparameter search.
Specifically, we use AdamW~\parencite{loshchilov2019decoupledweightdecayregularization} with a learning rate of $5 \times 10^{-6}$, linear learning-rate decay without warmup, no weight decay, a maximum sequence length of 256 tokens, and a batch size of 16.
Whereas \textcite{piskorski-etal-2024-cross} trained for 5 epochs, we train each model for up to 20 epochs and select the checkpoint with the highest validation F1-score, reducing the risk of underfitting under this conservative learning rate.
These optimisation settings are held fixed across all experimental conditions so that the controlled comparisons isolate changes in training-data composition; however, they were not independently optimised for \texttt{mmBERT}, and the absolute scores may depend on the optimisation recipe.
...
For the balanced shared \texttt{Multi-8} and \texttt{Multi-12} models and for the full-pool shared models, checkpoint selection is based on macro-averaged validation-set F1-score over the eight evaluation languages in $L_8$.
This also applies to the 12-language models: the four auxiliary languages in $L_{12} \setminus L_8$ contribute training data but do not enter the checkpoint-selection criterion.
...
The exception is Serbian, for which \texttt{Multi8-full-sr} essentially matches \texttt{Multi-8} (98.36 versus 98.38 F1).
This near-parity is expected because Serbian is the smallest dataset in $L_8$ and therefore determines the balanced quota $T_8$: \texttt{Multi-8} already retains all Serbian training data effectively, so the full-target variant adds no target-language evidence.
Serbian is also the only language for which \texttt{Multi-8} already outperforms \texttt{Mono-L}.
...
Their automatically constructed annotations, high annotation density, and short sentence segmentation make them a distinct source of supervision, and treating them as equally weighted gold data hurts in the balanced \texttt{Multi-12} setting.
...
%Fourth, language addition and corpus provenance are confounded because the four auxiliary languages are represented only by \texttt{WikiANN/PAN-X}; moreover, their inclusion in balanced \texttt{Multi-12} lowers the per-language quota for the main languages.
%The experiments therefore cannot isolate whether the balanced-setting degradation is caused by annotation quality, annotation-density and sentence-segmentation mismatch, reduced main-language evidence, or a combination of these factors.
Future work should separate corpus quality from language count more directly through a controlled ablation comparing the addition of a manually curated corpus for one further language, where such a resource is available, with the addition of a \texttt{WikiANN/PAN-X} corpus under a matched token budget and sampling procedure.
This comparison would provide a more direct test of whether the observed degradation is associated with expanding the language inventory or with the provenance and annotation characteristics of the auxiliary data.
...
Adding four lower-confidence auxiliary datasets in the balanced \texttt{Multi-12} setting consistently reduced performance.
However, the added languages are represented exclusively by automatically constructed \texttt{WikiANN/PAN-X} corpora whose annotation density and sentence segmentation differ markedly from those of the main corpora.
The result therefore argues against treating heterogeneous auxiliary corpora as interchangeable gold supervision, rather than showing that a larger language inventory is harmful in itself.
